...
contract LevCompStrategy is BaseLevFarmingStrategy {
    ...
    function _initStrategy(address _asset, address _cToken) internal {
        ...
        require(cToken.underlying() == address(asset));
    ...
    function setCollatRatios(uint256 _targetCollatRatio, uint256 _maxBorrowCollatRatio, uint256 _maxCollatRatio) external override onlyManagement {

        (, uint256 collateralFactorMantissa, ) = compound.markets(
            address(cToken)
        );

        require(_targetCollatRatio <= collateralFactorMantissa);
        require(_maxBorrowCollatRatio <= collateralFactorMantissa);
        require(_maxCollatRatio <= collateralFactorMantissa);
    ...
    function _estimateTokenToAsset(
        ...
        address[] memory path = getTokenOutPathV2(_from, address(asset));
